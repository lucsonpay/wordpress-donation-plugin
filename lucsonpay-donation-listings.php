<?php
ob_start();
function wp_lucsonpay_donation_listings_page() {
?>

<div>
	<h1>lucsonpay Payment Details</h1>
		<table cellpadding="0" cellspacing="0" bgcolor="#ccc" width="99%">
			<tr>
				...
				<?php
					...
					$total = $wpdb->get_var("SELECT COUNT(id)  FROM " . $wpdb->prefix . "lucsonpay_donation");
					
					$records_per_page = 10;
					$page = isset( $_GET['cpage'] ) ? abs( (int) $_GET['cpage'] ) : 1;
					$offset = ( $page * $records_per_page ) - $records_per_page;
					
					$donationEntries = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "lucsonpay_donation order by date desc limit ".$offset. " , ".$records_per_page);
