<?php
/**
 * Plugin Name: lucsonpay Payment Donation
 * Plugin URI: https://github.com/lucsonpay/
 * Description: This plugin allow you to accept donation payments using lucsonpay. This plugin will add a simple form that user will fill, when he clicks on submit he will redirected to lucsonpay website to complete his transaction and on completion his payment, lucsonpay will send that user back to your website along with transactions details. This plugin uses server-to-server verification to add additional security layer for validating transactions. Admin can also see all transaction details with payment status by going to "lucsonpay Payment Details" from menu in admin.
 * Version: 1.0
 * Author: lucsonpay
 * Author URI: http://lucsonpay.com/
 * Text Domain: lucsonpay Payments
 */

//ini_set('display_errors','On');
register_activation_hook(__FILE__, 'lucsonpay_activation');
register_deactivation_hook(__FILE__, 'lucsonpay_deactivation');

// do not conflict with WooCommerce lucsonpay Plugin Callback
if(!isset($_GET["wc-api"])){
	add_action('init', 'lucsonpay_donation_response');
}

add_shortcode( 'lucsonpaydonation', 'lucsonpay_donation_handler' );
...
if(isset($_GET['donation_msg']) && $_GET['donation_msg'] != ""){
	add_action('the_content', 'lucsonpayDonationShowMessage');
}

function lucsonpayDonationShowMessage($content){
...
function my_custom_fonts() {
  echo '<style>
   .toplevel_page_lucsonpay_options_page img{
      width:19px;
    } 
  </style>';
}
...
function lucsonpay_activation() {
	global $wpdb, $wp_rewrite;
	$settings = lucsonpay_settings_list();
	foreach ($settings as $setting) {
		add_option($setting['name'], $setting['value']);
	}
	add_option( 'lucsonpay_donation_details_url', '', '', 'yes' );
	$post_date = date( "Y-m-d H:i:s" );
	$post_date_gmt = gmdate( "Y-m-d H:i:s" );

	$ebs_pages = array(
		'lucsonpay-page' => array(
			'name' => 'lucsonpay Transaction Details page',
			'title' => 'lucsonpay Transaction Details page',
			'tag' => '[lucsonpay_donation_details]',
			'option' => 'lucsonpay_donation_details_url'
		),
	);
	
	$newpages = false;
	
	$lucsonpay_page_id = $wpdb->get_var("SELECT id FROM `" . $wpdb->posts . "` WHERE `post_content` LIKE '%" . $lucsonpay_pages['lucsonpay-page']['tag'] . "%'	AND `post_type` != 'revision'");
	if(empty($lucsonpay_page_id)){
		$lucsonpay_page_id = wp_insert_post( array(
			'post_title'	=>	$lucsonpay_pages['lucsonpay-page']['title'],
			'post_type'		=>	'page',
			'post_name'		=>	$lucsonpay_pages['lucsonpay-page']['name'],
			'comment_status'=> 'closed',
			'ping_status'	=>	'closed',
			'post_content' =>	$lucsonpay_pages['lucsonpay-page']['tag'],
			'post_status'	=>	'publish',
			'post_author'	=>	1,
			'menu_order'	=>	0
		));
	...
	update_option( $lucsonpay_pages['lucsonpay-page']['option'], _get_page_link($lucsonpay_page_id) );
	unset($lucsonpay_pages['lucsonpay-page']);
	
	$table_name = $wpdb->prefix . "lucsonpay_donation";
...
function lucsonpay_deactivation() {
	$settings = lucsonpay_settings_list();
	foreach ($settings as $setting) {
...
function lucsonpay_settings_list(){
	$settings = array(
		array(
			'display' => 'Merchant ID',
			'name'    => 'lucsonpay_merchant_id',
			'value'   => '',
			'type'    => 'textbox',
			'hint'    => 'Merchant Id Provided by lucsonpay'
		),
		array(
			'display' => 'Merchant Key',
			'name'    => 'lucsonpay_merchant_key',
			'value'   => '',
			'type'    => 'textbox',
			'hint'    => 'Merchant Secret Key Provided by lucsonpay'
		),
		array(
			'display' => 'Website',
			'name'    => 'lucsonpay_website',
			'value'   => '',
			'type'    => 'textbox',
			'hint'    => 'Website Name Provided by lucsonpay'
		),
		array(
			'display' => 'Industry Type',
			'name'    => 'lucsonpay_industry_type_id',
			'value'   => '',
			'type'    => 'textbox',
			'hint'    => 'Industry Type Provided by lucsonpay'
		),
		array(
			'display' => 'Channel ID',
			'name'    => 'lucsonpay_channel_id',
			'value'   => 'WEB',
			'type'    => 'textbox',
			'hint'    => 'Channel ID Provided by lucsonpay e.g. WEB/WAP'
		),
		array(
			...
			'value'   => '',
			'type'    => 'textbox',
			'hint'    => 'Transaction URL Provided by lucsonpay'
		),
		array(
			...
			'value'   => '',
			'type'    => 'textbox',
			'hint'    => 'Transaction Status URL Provided by lucsonpay'
		),
		array(
			'display' => 'Default Amount',
			'name'    => 'lucsonpay_amount',
			'value'   => '100',
			'type'    => 'textbox',
...
if (is_admin()) {
	add_action( 'admin_menu', 'lucsonpay_admin_menu' );
	add_action( 'admin_init', 'lucsonpay_register_settings' );
}


function lucsonpay_admin_menu() {
	add_menu_page('lucsonpay Donation', 'lucsonpay Donation', 'manage_options', 'lucsonpay_options_page', 'lucsonpay_options_page', plugin_dir_url(__FILE__).'assets/logo.ico');
	//, plugin_dir_url(__FILE__).'assets/logo.ico'

	add_submenu_page('lucsonpay_options_page', 'lucsonpay Donation Settings', 'Settings', 'manage_options', 'lucsonpay_options_page');

	add_submenu_page('lucsonpay_options_page', 'lucsonpay Donation Payment Details', 'Payment Details', 'manage_options', 'wp_lucsonpay_donation', 'wp_lucsonpay_donation_listings_page');
	
	require_once(dirname(__FILE__) . '/lucsonpay-donation-listings.php');
}


function lucsonpay_options_page() {
	echo	'<div class="wrap">
				<h1>lucsonpay Configuarations</h1>
			...
			$last_updated = "";
			$path = plugin_dir_path( __FILE__ ) . "/lucsonpay_version.txt";
			if(file_exists($path)){
				$handle = fopen($path, "r");
				if($handle !== false){
...
function lucsonpay_register_settings() {
	$settings = lucsonpay_settings_list();
	foreach ($settings as $setting) {
...
function lucsonpay_donation_handler(){

	if(isset($_REQUEST["action"]) && $_REQUEST["action"] == "lucsonpay_donation_request"){
		return lucsonpay_donation_submit();
	} else {
		return lucsonpay_donation_form();
	}
}

function lucsonpay_donation_form(){
	...
	$html = '<form name="frmTransaction" method="post">
					...
					<p>
						...
						<input type="text" name="donor_amount" maxlength="10" value="'.trim(get_option('lucsonpay_amount')).'" required />
					</p>
					<p>
					...
					<p>
						<input type="hidden" name="action" value="lucsonpay_donation_request">
						<input type="submit" value="' . trim(get_option('lucsonpay_content')) .'" required />
					</p>
				</form>';
...
function lucsonpay_donation_submit(){
	...
	if($valid){
		...
		//End Post Request
       //echo get_option('lucsonpay_merchant_id');exit;
 		$post_params = array(
			'PAY_ID' => trim(get_option('lucsonpay_merchant_id')),
				'ORDER_ID' =>$order_id,
		...
		$post_params["HASH"] = PayDonation::getHashFromArray(	$post_params,
																				trim(get_option('lucsonpay_merchant_key'))
																			);
...
function lucsonpay_donation_meta_box() {
	$screens = array( 'lucsonpaydonation' );
	
	foreach ( $screens as $screen ) {
		add_meta_box(  'myplugin_sectionid', __( 'lucsonpay', 'myplugin_textdomain' ),'lucsonpay_donation_meta_box_callback', $screen, 'normal','high' );
	}
}

add_action( 'add_meta_boxes', 'lucsonpay_donation_meta_box' );

function lucsonpay_donation_meta_box_callback($post) {
	echo "admin";
}

function lucsonpay_donation_response(){
	global $wpdb;
	...
	if($_POST['STATUS']=="Captured")
{
		...
		$lucsonpay_merchant_key = trim(get_option('lucsonpay_merchant_key'));
		$lucsonpay_merchant_id = trim(get_option('lucsonpay_merchant_id'));
   ...
   $msg = "Thank you for your order. Your transaction has been successful.";
					$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix . "lucsonpay_donation SET payment_status = 'Complete Payment' WHERE  id =%d", sanitize_text_field($_POST['ORDER_ID'])));
				
				}
// 				else{

// $msg = "It seems some issue in server to server communication. Kindly connect with administrator.";
// 					$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix . "lucsonpay_donation SET payment_status = 'Fraud Payment' WHERE id = %d", sanitize_text_field($_POST['ORDER_ID'])));
// 				}
		...
		// if(lucsonpayDonation::verifychecksum_e($_POST, $lucsonpay_merchant_key, $_POST['CHECKSUMHASH']) === "TRUE") {
			
		// 	if($_POST['RESPCODE'] == "01"){

		// 		// Create an array having all required parameters for status query.
		// 		$requestParamList = array("MID" => $lucsonpay_merchant_id, "ORDERID" => $_POST['ORDERID']);

		// 		// $_POST['ORDERID'] = substr($_POST['ORDERID'], strpos($_POST['ORDERID'], "-") + 1); // just for testing
				
		// 		$StatusCheckSum = lucsonpayDonation::getChecksumFromArray($requestParamList, $lucsonpay_merchant_key);

		// 		$requestParamList['CHECKSUMHASH'] = $StatusCheckSum;

		// 		$responseParamList = lucsonpayDonation::callNewAPI($transaction_status_url, $requestParamList);

		// 		if($responseParamList['STATUS'] == 'TXN_SUCCESS' && $responseParamList['TXNAMOUNT'] == $_POST['TXNAMOUNT']) {
		// 			$msg = "Thank you for your order. Your transaction has been successful.";
		// 			$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix . "lucsonpay_donation SET payment_status = 'Complete Payment' WHERE  id = %d", sanitize_text_field($_POST['ORDERID'])));
				
		// 		} else  {
		// 			$msg = "It seems some issue in server to server communication. Kindly connect with administrator.";
		// 			$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix . "lucsonpay_donation SET payment_status = 'Fraud Payment' WHERE id = %d", sanitize_text_field($_POST['ORDERID'])));
		// 		}

		// 	} else {
		// 		$msg = "Thank You. However, the transaction has been Failed For Reason: " . sanitize_text_field($_POST['RESPMSG']);
		// 		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix . "lucsonpay_donation SET payment_status = 'Cancelled Payment' WHERE id = %d", sanitize_text_field($_POST['ORDERID'])));
		// 	}
		// } else {
		// 	$msg = "Security error!";
		// 	$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix . "lucsonpay_donation SET payment_status = 'Payment Error' WHERE  id = %d", sanitize_text_field($_POST['ORDERID'])));
		// }
...
* Code to test Curl
*/
if(isset($_GET['lucsonpay_action']) && $_GET['lucsonpay_action'] == "curltest"){
...
function curltest_donation($content){
	...
	$content = "<center><h1>cURL Test for lucsonpay Donation Plugin</h1></center><hr/>";
